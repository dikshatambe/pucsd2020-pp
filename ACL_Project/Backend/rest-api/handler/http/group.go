...
func (group *Group) GetHTTPHandler() []*handler.HTTPHandler {
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodGet, Path: "group/{gid}", Func: group.GetByID},
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodPost, Path: "group", Func: group.Create},
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodPut, Path: "group/{gid}", Func: group.Update},
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodDelete, Path: "group/{gid}", Func: group.Delete},
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodGet, Path: "group", Func: group.GetAll},
		&handler.HTTPHandler{Authenticated: false, Method: http.MethodPatch, Path: "group/{gid}", Func: group.Update},
	}
}
...
func (group *Group) Update(w http.ResponseWriter, r *http.Request) {
	var iUsr interface{}
	id,_ := strconv.ParseInt(chi.URLParam(r, "gid"), 10, 64)
	grp := model.Group{}
	err := json.NewDecoder(r.Body).Decode(&grp)
	for {
		if nil != err {
